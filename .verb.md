...
- full compatibility with `co@4` and passing 100% of its tests (works as drop-in replacement)
...
### Working on old environment

Note that `{%= name %}` will work out-of-the-box in most cases and without problems. But in old environment which don't have support for native Promises there's little thing that you should remember.
